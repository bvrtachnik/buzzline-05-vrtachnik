...
## Custom Consumer Terminal: consumer_vrtachnik.py

As part of this project, I created a new consumer named **`consumer_vrtachnik.py`**.  
This consumer reads from the live data file, processes each message individually, and stores results in **SQLite**.  

### Focus and Insight

For each message, the consumer calculates additional insights beyond the raw JSON:  

- **Sentiment Label**: The numeric sentiment score is mapped into a human-readable category (*Very Negative, Negative, Neutral, Positive, Very Positive*). This makes trends in tone easier to interpret at a glance.  
- **Engagement Score**: A derived metric that multiplies the sentiment intensity by the message length. This acts as a simple proxy for how impactful or engaging a message might be — longer, more strongly polarized messages receive higher scores.  

These additions provide immediate analytical value by transforming raw sentiment data into insights that can be aggregated and compared later.

### How to Run

This project requires two terminals: one for the producer and one for the consumer.
...
#### **Terminal 2 - Custom Consumer**
```
.\.venv\Scripts\Activate.ps1
py -m consumers.consumer_vrtachnik
```
The consumer will process each incoming message, classify its sentiment, calculate its engagement score, and insert the enriched record into ```buzz.sqlite```.

### Verification
To verify, check the SQLite database after running both producer and consumer: 
```
sqlite3 data/buzz.sqlite "SELECT message, sentiment, sentiment_label, engagement_score FROM streamed_messages LIMIT 10;"
```

You should see rows that include both the numeric sentiment and the derived ```sentiment_label``` and ```engagement_score```.

### Verification checklist
- Producer terminal is actively generating JSON messages. 
- Consumer terminal shows processed messages with **sentiment_label** and **engagement_score**.
- SQLite database contains enriched rows with calculated insights.
